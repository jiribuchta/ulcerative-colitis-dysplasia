...
class EmbedTiles:
    def __init__(self, model: str, concurrency: int) -> None:
        ...
        self.client = AsyncClient(
            models_base_url="http://rayservice-model-serve-svc.rationai-jobs-ns.svc.cluster.local:8000",
            limits=httpx.Limits(
...
def main(config: DictConfig, logger: MLFlowLogger) -> None:
    for name, split_uri in config.dataset.mlflow_uris.tiling_filtered.items():
        ...
        slide_info = slides.set_index("id")[["path", "level", "tile_extent_x", "tile_extent_y"]]
